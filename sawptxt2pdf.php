...
function pokaz_head() {
	...
	<style type="text/css">
		.linie { font-family: Courier New,Courier; font-size:12px; };
		small { font-size:10px; }
		.kom { color: #c0c0c0; }
		.o { color: black; }
		.u { font-weight: bold; }
		.n { font-weight: bold; color: darkred; }
		.a { font-weight: bold; color: navy; }
		.d, .t, .w { color: #909090; }
		.dolar { color: #B8860B; }
		H1 { margin-bottom:0px; }
		.wersja { margin-bottom:5px; }
		TEXTAREA#posttresc {
			width:100%;
			background-color:#d0d0d0;
			height: calc(100% - 175px);
			min-height:100px; }
		#stopka { background-color:#f0f0f0; }
		INPUT#wszystkofile { width:50%; background-color:#e0e0e0; }
	</style>
</head>';
};
...
function pokaz_naglowek() {
  ...
  echo '<h1>SAWPFORM - beta '.$SAWPTXT2PDFwersja.'</h1>';
  echo '<div class="wersja">['.$SAWPTXT2PDFdata.'] &copy; <a href="http://kamilbaranski.com/">kamilbaranski</a></div>';
  echo '<a name="top"></a>';
};

function skrypt_dragndrop() {
...
var holder = document.getElementById('posttresc'),
...
function pokaz_interfejs() { 
	...
	echo '<form method="post" id="formularz"><textarea id="posttresc" name="posttresc">';
	echo file_get_contents('_opis.txt');
	echo '</textarea><br />';
    echo '<div id="stopka">';
	echo '<input name="wszystkofile" id="wszystkofile" value="sawp_.pdf" /><br />';
	echo '<div id="status" name"status"><span style="color:darkred">(Drag&drop chyba nie da rady.)</span></div>';
	echo '<input name="download" id="download" type="checkbox" value=false> Ściągnąć? <span style="color:#c0c0c0;">(inaczej spróbuje otworzyć PDF w przeglądarce)</span>.<br />';
	echo '<input name="generujodrazu" id="generujodrazu" type="checkbox" value=false> Generuj od razu po upuszczeniu?<br />';
	echo '<input type="submit" value="Generuj" /></form>';
	echo '</div>';
...
function implode2($co,$ile) {	// łączy nową linią, chyba że więcej niż $ile - wtedy łączy przecinkiem.
	if (count($co)>$ile) { return implode(', ',$co); } else { return implode("\n",$co); };
};
...
class piosenka {		// p jak piosenka
	...
	function dodajlinie($ludzie,$funkcje) {
	  ...
	  foreach($ludzie as $czlowiek) {
	    if($czlowiek=='kb') { $czlowiek='Kamil Barański'; };
	    if($czlowiek=='kb!') { $czlowiek='Kamil Barański!'; };
...
function inicjuj_pdf() {
    ...
    $czcionka='czcionka'; $czcionkafile='kubareczny2005.php';
    $czcionka='TimesNewRomanPSMT'; $czcionkafile='timesnewromanpsmt.php';
